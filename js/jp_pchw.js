var rule = {
    ...
    二级: {
        ...
        tabs: '道长在线',
        lists: "js:let d=[];let html=request(input);let playUrls=[];let playNames=[];let m=html.match(/g_PlayUrl='([\\s\\S]*?)';/);if(m && m[1] && m[1].length>0){let arr=m[1].split('#');for(let i=0;i<arr.length;i++){let kv=arr[i].split('$');if(kv.length==2){playNames.push(kv[0]);playUrls.push(kv[1]);}}}if(playUrls.length==0){let scriptMatch=html.match(/<script[^>]*>g_PlayUrl='([\\s\\S]*?)';<\\/script>/);if(scriptMatch && scriptMatch[1] && scriptMatch[1].length>0){let arr=scriptMatch[1].split('#');for(let i=0;i<arr.length;i++){let kv=arr[i].split('$');if(kv.length==2){playNames.push(kv[0]);playUrls.push(kv[1]);}}}}for(let i=0;i<playUrls.length;i++){d.push(playNames[i]||'播放'+(i+1)+'$'+playUrls[i]);}if(d.length==0){d.push('播放$https://www.baidu.com')} setResult([d]);"
    },
